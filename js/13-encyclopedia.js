// 13-encyclopedia：怪物图鉴数据 / UI / 形态预览绘制
  ...
  const ENCY_DATA = {
    ...
    striker_dusk: {
      ...
      desc: '在场地 30%~80% 高度的随机位置上方<b>渐显浮现</b>，下移停驻 <b>0.2s</b>（白环从核心掠过机体至边缘消失作预警），<b>白环散尽的同时</b>以随机方向为基准向四周<b>正六边形或正八边形（随机）</b>的均匀方向各射 1 发，随即下移同距<b>渐隐离场</b>（浮现到开火全程约 <b>2s</b>）。<b>无法碰撞</b>：与玩家互相穿过、不受撞机反伤。<b>80% 掉落 3~5 个水晶</b>。出现概率：Lv11 前 <b>约 1.9%</b>（权重 2/107）、Lv11 起 <b>12.5%</b>（权重 5/40）。',
    },
    gunship_violet: {
    ...
    weilong: {
      name: '威龙', type: 'weilong', color: '#ff9a1a', hp: 5000, score: 1200,
    ...
    fashiA1: {
      name: '法术大师A1', type: 'fashiA1', color: '#a855f7', hp: 80, score: 180,
    ...
    capital_crimson: {
      name: '赤红主力舰', type: 'capital', variant: 'crimson', color: '#ff4d6d', hp: 4200, score: 1500,
    ...
    capital_azure: {
      name: '苍蓝主力舰', type: 'capital', variant: 'azure', color: '#4d9fff', hp: 4200, score: 1500,
    ...
    capital_crgold: {
      name: '赤金主力舰', type: 'capital', variant: 'crgold', color: '#ff9a1a', hp: 4200, score: 1500,
    ...
    boss: {
      name: '旧日之歌', type: 'boss', color: '#e6d5ff', hp: 36000, score: 6000, bossId: 'song',
...
boss_storm: {
      ...
      diffNotes: {
        xuxiang: 'BOSS 伤害 <b>-40%</b>；技能释放间隔 <b>+50%</b>；<b>不会连续释放同种技能</b>。技能组与具象一致。',
        zhenwo: '<b>技能1</b>：脱离技能轮换——每 10~16s <b>独立释放</b>一轮风波（单轮 3~4 道、随机一侧），不占用技能槽、不影响技能释放间隔。<b>技能2</b>：大型龙卷血量 <b>6000</b>（具象 3600），受僚机伤害加成额外 +150%（与基础加算、不乘算）。<b>技能3</b>：共 <b>6 轮</b>射击，每轮同时射出 <b>2 个风柱</b>（位置至少相差 10% 屏宽），轮间隔 +50%。<b>技能4</b>：总时长 <b>9s</b>，期间自身减伤 25%、旋转速度 +40%、风弹射速 +60%、风弹长度 +30%；初始方向顺/逆时针随机，期间随机改变 2~3 次方向。<b>技能5</b>：两轮风弹 <b>14/11</b> 发；普通风弹 20% 概率射速减慢 20%~50%（强化大风弹不减慢）。<b>技能6</b>：追加一组<b>镜像三旋臂</b>（两射击点关于竖直中轴精确镜像、转向相反，任意时刻保持镜像）。<b>技能7</b>：涡流风旋改为<b>三旋臂</b>，风弹射速 +25%、最大转速 2.3 rad/s（具象 2.625）。另有<b>技能8「双子旋臂」</b>加入技能池——距风暴中心 30%~80% 半径环内随机两点（间距 ≥70px）绕中心公转，各以 50% 概率发出三旋臂或四旋臂，持续 6s（旋臂自转方向随机且两点独立）。',
      },
    },
    boss_storm2: {
      name: '风暴编织者', type: 'boss', color: '#8fd4ff', hp: 36000, score: 6000, bossId: 'storm2',
    ...
    escort: {
      ...
      desc: '增生侧翼艇阵亡时分裂出的衍生体（每艘分裂 <b>0~3</b> 个）：<b>深蓝紫渐变小三角、边缘泛紫色光芒</b>（与水晶的浅蓝明显区分）。<b>无攻击</b>，沿母舰原航向漂移；碰撞 4.8、造成的无敌时间仅为常规的 <b>40%</b>（0.48s）。<b>不掉落任何水晶与道具</b>（守愿者弹对其无限穿透）。不计入场面压力。',
    },
    tornado: {
